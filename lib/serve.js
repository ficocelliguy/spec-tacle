...
function buildInventory(kind, source, descriptions) {
  ...
  if (family === 'flowchart' || /^(architecture|user flow|dependency map)$/i.test(kind || '')) {
...
function makeConsistencyPendingHook({ commandTemplate, port, logPrefix, broadcast }) {
  ...
  return (entry, { servedRoot }) => {
    ...
    // hook fires — headless `claude -p` buffers stdout until it flushes at
    // exit, so relying on the child to POST /consistency-claim itself means
    // the UI paints "stalled" through the whole (successful) pass. The auto
    ...
    try {
      const updated = updateConsistencyEntry(servedRoot, entry.id, (e) => {
        ...
        // the moment the subprocess is spawned. Headless `claude -p` buffers
        // stdout until it flushes at exit, so it won't POST /consistency-
        // progress mid-run — without this seed the bar sits at 0 through the
        // whole (successful) pass and the operator sees no motion.
        const seededProgress = {
          percent: 5,
          phase: 'auto-agent spawning',
          note: 'headless subprocess; output flushes at completion',
          updatedAt: now,
        };
    ...
    // Log the exit code + a duration hint so a fast-failing subprocess (auth,
    // missing binary, model refused) surfaces on the server stdout — the
    // operator learns to check the log file. A pass that succeeds ends via
    // /consistency-apply, which clears the queue, so seeing "exit 0" here
    // without a matching "consistency-pass applied" line means the subprocess
    // finished without applying anything (also worth knowing).
    const spawnedAt = Date.now();
    child.on('exit', (code, signal) => {
      const dur = ((Date.now() - spawnedAt) / 1000).toFixed(1);
      const how = signal ? `signal ${signal}` : `exit ${code}`;
      const tail = logPath ? ` — tail ${path.relative(servedRoot, logPath)}` : '';
      process.stdout.write(`${label} auto-agent for ${entry.id} finished (${how}, ${dur}s)${tail}\n`);
      if (logStream != null) { try { fs.closeSync(logStream); } catch (_) { /* not fatal */ } }
      // If the entry is still in the queue AND still held by the auto-claim
      // we stamped, the subprocess died without applying — release the claim
      // so the visualizer flips out of "active" and the operator can retry
      // (or a manual pass can claim). If the entry was cleared by
      // /consistency-apply (success path) or a manual /consistency-release
      // took over the claim, there's nothing for us to do.
        ...
        try {
          ...
          if (released && updated) {
            broadcast('consistency-released', {
              specPath: updated.specPath,
              entryId: updated.id,
              entry: decorateEntry(updated, Date.now()),
            });
          }
        } catch (_) { /* not fatal */ }
      }
...
function defaultAutoAgentCommand() {
  ...
  const prompt = [
    ...
    'Served root: {root}',
    'Server: http://127.0.0.1:{port}',
    'Steps: claim the entry, report progress at 25/55/85%, apply follow-up edits via /consistency-apply (and Edit for non-marker prose), re-sync the data JSON, re-render the HTML, then let apply clear the queue.',
    ...
    'If you cannot make a confident set of edits, POST /consistency-release with the entry id and exit.',
  ].join(' ');
  return `${bin} -p ${JSON.stringify(prompt)} --model ${model}`;
}
...
function start({ root = '.', port = 8765, portRetries = 10, onListen, onConsistencyPendingCmd, autoAgent } = {}) {
  ...
  const announceReady = () => {
    ...
    process.stdout.write('GET  /events               SSE — `spec-changed`, `consistency-pending`, `consistency-claimed`, `consistency-progress`, `consistency-released`, `consistency-applied`, `consistency-dismissed`\n');
