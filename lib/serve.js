...
function applyUpdates(original, payload) {
  ...
  for (const [name, entry] of Object.entries(diagrams)) {
    // Two accepted shapes per diagram:
    //   "source"                       → wrap in a ```mermaid``` fence (legacy)
    //   { source, kind }               → wrap based on kind: 'table' writes the
    //                                    source as raw markdown (no fence);
    //                                    everything else wraps in ```mermaid```.
    ...
    const body = kind === 'table' ? source : ('```mermaid\n' + source + '\n```');
